...
| `binbin-contentfly-wechat-article` | 彬彬公众号文章 | 三步完成逐字稿转公众号 Markdown、公众号 HTML 排版和公众号封面 |
...
精修工作流会在运行前检查 dbskill 和 renwei-writing。完整发布工作流会分阶段调用 `dbs-xhs-title`、`binbin-contentfly-publish-copy` 和 `binbin-contentfly-cover`，每阶段等待用户确认。发布文案工作流会先调用 `binbin-contentfly-risk-check`，再调用 `renwei-writing`；公众号文章工作流会用 `khazix-writer` 校准真人感，随后调用 `gzh-design` 完成公众号排版，再调用 `baoyu-cover-image` 生成公众号封面；图片和视频位置允许保留简短无框文字标记，不必等待素材补齐。公众号文章工作流首次运行会检查这三个依赖，缺失时只从用户指定的 `KKKKhazix/khazix-skills`、`isjiamu/gzh-design-skill` 与 `JimLiu/baoyu-skills` 自动安装，再复检；已安装时不覆盖。风险检查已内置彬彬视频违禁词规范，但通用语义风险检查仍由 dbskill 中的 `dbs-content-risk-check` 提供。其他第三方依赖不会随本仓库打包，也不会未经用户同意自动安装。
